...
std::istream & comp::operator >> (std::istream & in, CalcMachine & item)
{
	do
	{
		item.validator.checkedNumericalInput(item.manufactureYear, "Input manufacture year: ");
	} while (item.manufactureYear < comp::MIN_YEAR || item.manufactureYear > comp::MAX_YEAR);

	do
	{
		item.validator.checkedNumericalInput(item.compCode, "Input code: ");
	} while (item.compCode < 0);

	item.operatingSystem = item.validator.checkedInput("Input OS: ");
